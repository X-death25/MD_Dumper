int Read_ROM_Auto(void)
{
	...
	if ( sms_mode == 0 && Hardwaretype == 0 ) // Dump Megadrive cartridge in no mapper mode
	{
		...
		// Cleaning ROM Buffer
		for (i=0; i<game_size; i++)		BufferROM[i]=0x00;
	...
	else if ( sms_mode == 0 && Hardwaretype == 3 ) // Automatic Dump Megadrive cartridge in Lock-on mapper mode
	{
		...
		else if (gg_mode == 1 )		SDL_Log("GAME GEAR Mode : ROM dump in progress...\n");
		...
		else if (gg_mode == 1 )		myfile = fopen("dump_rom.gg","wb");
		fwrite(BufferROM, 1,game_size, myfile);
		fclose(myfile);
	}
	return 0;
...
int Read_RAM_Auto(void)
{
	...
	for (i=0; i<(save_size*2); i++)		BufferSAVE[i]=0x00;
...
int Read_RAM_Manual(void)
{
	...
	timer_start();
	if(dump_sram_size_opts==0)			save_size = 8192;
	else if(dump_sram_size_opts==1)		save_size = 32768;
	else								save_size *= 1024;
